...
class Scrape():
        ...
        for url in page_urls:
            ...
            try:
                ...
                for cell in all_cells:
                    all_links.append(('/').join(self.base_url.split('/')[:-2]) + '/' + cell.a.get('href').split('/')[1:][0])
            except:
                pass  # malformity
    ...
    def prepare_content(self, content, detail_url):
        ...
        # add credit
        content += '<p>More info and image formats at <a target = "_blank" href = "%s">%s</a></p>' % (detail_url, detail_url)
...
class Publish():
    ...
    def post_to_wordpress(self, title, content, detail_url, local_img_file, previously_published, retry):
        ...
        except:  # occasionally response is 404, wait and try again
            if retry:
                print "sleep 3"
                sleep(3)
                return self.post_to_wordpress(title, content, detail_url, local_img_file, previously_published, False)
            else:
