...
publish = Publish(published_url=published_url, previously_published=previously_published)
...
for detail_url in all_links:
    ...
    if img_id not in previously_published:
        ...
        try:
            (title, content, detail_url, local_img_file) = scrape.grab_content_from_page(detail_url)
        except:  # guh
            print("nope")
            continue  # move along
        print("posting to WP: " + title)

        publish.post_to_wordpress(title, content, detail_url, local_img_file, previously_published, True)

if not found:
    print "all links are previously published"
print("Bye!")